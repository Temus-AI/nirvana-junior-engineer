...
def parent_selection(
    pop: List[EvolNode], m: int, proportion: float = 0.8
) -> List[dict]:
    """
    ...
    selected_parents = []
    tournament_size = min(
        len(pop) // m, int(len(pop) * proportion)
...
class Evolution:
    def __init__(
        ...
        if not plan:
            ...
            self.strategy_trace = (
                "Initial Population information: " + self.population_info
            )
        else:
            pass
    ...
    def _get_offspring(
        self, operator, pop: list = [], max_attempts: int = -1, feedback: str = ""
    ):
        """
        ...
        if operator == "i1":  # initialization operator
            ...
            offspring["reasoning"], offspring["code"], offspring["fitness"] = (
                self.evol.reasoning,
                self.evol.code,
                self.evol.fitness,
            )
            ...
            assert (
                len(pop) >= self.num_parents
            ), "Population size is less than the number of parents required for Crossover operator"
            parents = parent_selection(
                pop, self.num_parents
            )  # in fact we don't mind 3P
            self.evol.evolve(
                operator,
                parents,
                replace=True,
            ...
            assert (
                len(pop) >= 1
            ), "Population size is less than the number of parents required for Mutation operator"
    ...
    def get_offspring(
        self,
        method: Union[str, List[str]] = "default",
        convert_to_plan: bool = False,
        feedback: str = "",
    ):
        if isinstance(method, str):
            self.population = self._get_offspring(
                method, self.population, feedback=feedback
            )
        elif isinstance(method, list):
            for m in method:
                self.population = self._get_offspring(
                    m, self.population, feedback=feedback
                )
        if convert_to_plan and (
            (len(self.population) == 0)
            or (
                max(self.population, key=lambda x: x["fitness"])["fitness"]
                < self.plan_threshold
            )
        ):
            print("Converting to Plan")
            meta_plan = MetaPlan(
                self.meta_prompt.task,
                self.meta_prompt.func_name,
                self.meta_prompt.inputs,
                self.meta_prompt.outputs,
                self.meta_prompt.input_types,
                self.meta_prompt.output_types,
            )
    ...
    def save_population(
        ...
        pop: list,
        filename: str = "default",
        population_dir: str = "methods/population",
    ):
    ...
    def load_population(
        self, filename: str = "default", population_dir: str = "methods/population"
    ):
        population_folder = f"{population_dir}/{self.meta_prompt.func_name}/"
        filepath = os.path.join(population_folder, filename + ".json")
    ...
    def chat(
        self,
        question: str = "How effective is the current evolution strategy? What improvement has it made in terms of fitness, and in terms of the implementation?",
        get_response: Optional[Callable] = None,
    ):
        prompt = f"{question}\n\n{self.strategy_trace}"
        response = (
            self.get_response(prompt) if get_response is None else get_response(prompt)
        )
