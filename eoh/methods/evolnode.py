...
from .meta_execute import (
    call_func_code,
    call_func_prompt,
    call_func_prompt_parallel,
    call_func_prompts,
    combine_errors,
    combine_scores,
    compile_code_with_references,
)
...
def map_input_output(
    test_case_list: List[dict], input_names: List[str], output_names: List[str]
...
def require_llm_metric(
    name: str, value: str, custom_metric_map: Optional[Dict[str, Callable]] = None
) -> bool:
...
def _check_alignment_with_metric(
    pred_output: dict,
    target_output: dict,
    ...
    for key, target_value in target_output.items():
        ...
        if not is_aligned:
            ...
            error_msg += (
                f"Value mismatch for key {key}: {pred_value} != {target_value}\n"
            )
            return False, error_msg
...
def _check_alignment_with_llm_sequential(
    pred_output: dict,
    target_output: dict,
    custom_metric_map: Optional[Dict[str, Callable]] = None,
    ...
    prompt = ALIGNMENT_CHECK_PROMPT.format(
        pred_output=pred_output, target_output=target_output
    )

    trimmed_pred_output = {
        k: v
        for k, v in pred_output.items()
        if require_llm_metric(k, v, custom_metric_map)
    }
    trimmed_target_output = {
        k: v
        for k, v in target_output.items()
        if require_llm_metric(k, v, custom_metric_map)
    }
    ...
    prompt = ALIGNMENT_CHECK_PROMPT.format(
        pred_output=trimmed_pred_output, target_output=trimmed_target_output
    )
    for i in range(max_tries):
        try:
            ...
            if check_dict["aligned"]:
                ...
                return (
                    False,
                    "--- LLM-Evaluation concludes prediction is not aligned with target output",
                )
...
def _check_alignment_with_metric_parallel(
    output_per_code_per_test: Dict[int, Dict[int, Dict]],
    ...
    for code_index in output_per_code_per_test:
        for test_index in output_per_code_per_test[code_index]:
            pred_output, target_output = (
                output_per_code_per_test[code_index][test_index],
                target_outputs[test_index],
            )
            ...
            if isinstance(pred_output, dict) and isinstance(target_output, dict):
                trimmed_pred = {
                    k: v
                    for k, v in pred_output.items()
                    if not require_llm_metric(k, v, custom_metric_map)
                }
                trimmed_target = {
                    k: v
                    for k, v in target_output.items()
            ...
            is_aligned, error_msg = _check_alignment_with_metric(
                pred_output, target_output, custom_metric_map
            )
...
def _check_alignment_with_llm_parallel(
    output_per_code_per_test: Dict[int, Dict[int, Dict]],
    errors_per_code_per_test: Dict[int, Dict[int, List[str]]],
    test_inputs: List[Dict],
    target_outputs: List[Dict],
    ...
    for code_index in output_per_code_per_test:
        for test_index in output_per_code_per_test[code_index]:
            ...
            for target_name, target_value in target_output.items():
                if (
                    require_llm_metric(target_name, target_value, custom_metric_map)
                    and target_name in pred_output
                ):
            ...
            prompt = ALIGNMENT_CHECK_PROMPT.format(
                pred_output=trimmed_pred, target_output=trimmed_target
            )
    ...
    # Get LLM responses
    desc_str = (
        f"Running LLM-based alignment check in parallel with batch size {len(prompts)}"
    )
    ...
    for i, response in enumerate(responses):
        ...
        try:
            check_dict = extract_json_from_text(response)
            scores_per_code_per_test[code_index][test_index].append(
                float(check_dict["aligned"])
            )
        except:
            errors_per_code_per_test[code_index][test_index].append(
                f"LLM Evaluation Failed: {response}"
            )
    ...
    for code_index in scores_per_code_per_test:
        for test_index in scores_per_code_per_test[code_index]:
            score = sum(scores_per_code_per_test[code_index][test_index]) / len(
                scores_per_code_per_test[code_index][test_index]
            )
...
def check_alignment_sequential(
    pred_output: dict,
    target_output: dict,
    get_response: Optional[Callable] = get_openai_response,
    max_tries: int = 3,
    ...
    _, error_msg_delta = _check_alignment_with_llm_sequential(
        pred_output, target_output, get_response, max_tries
    )
    error_msg += error_msg_delta
...
def check_alignment_parallel(
    ...
    metric_scores, errors_per_code_per_test = _check_alignment_with_metric_parallel(
        output_per_code_per_test,
        errors_per_code_per_test,
        test_inputs,
        target_outputs,
        custom_metric_map,
    )
    ...
    llm_scores, errors_per_code_per_test = _check_alignment_with_llm_parallel(
        output_per_code_per_test,
        errors_per_code_per_test,
        test_inputs,
        target_outputs,
...
def print_individual_info(
    code_index: int, fitness: float, err_msg: str, reasoning: str, code: str
):
    print("\n" + "=" * 80)  # Top separator
...
class EvolNode:
    ...
    def _extend_test_cases(self, num_cases: int = 5, feedback: str = ""):
        ...
        try:
            ...
            self.test_cases.extend(
                map_input_output(
                    test_case_list, self.meta_prompt.inputs, self.meta_prompt.outputs
                )
            )
    ...
    def _get_evolve_response_sequential(
        self, method: str, parents: Optional[list] = None, feedback: str = ""
    ):
        ...
        prompt_content += (
            "\nIdea: " + feedback
        )  # External Guidance (perhaps we should reddit / stackoverflow this thingy)

        response = self.get_response(prompt_content)
        return response

    def _get_evolve_prompt(
        self, method: str, parents: Optional[list] = None, feedback: str = ""
    ):
        prompt_method = getattr(self.meta_prompt, f"_get_prompt_{method}")
        prompt_content = prompt_method(parents)
    ...
    def _evolve(
        self, method: str, parents: list = None, feedback: str = "", batch_size: int = 5
    ):
        """
        ...
        for response in responses:
            try:
                ...
                code = compile_code_with_references(
                    code, self.referrable_function_dict
                )  # deal with node references
    ...
    def evolve(
        ...
        if query_node:
            self.query_nodes(
                ignore_self=replace, self_func_name=self.meta_prompt.func_name
            )
        ...
        # Evolve many times
        reasonings, codes = self._evolve(
            method, parents, feedback=feedback, batch_size=batch_size
        )
        ...
        self.codes = codes
        fitness_per_code, errors_per_code, global_summary = self._evaluate_fitness(
            codes=codes,
            max_tries=max_tries,
            num_runs=num_runs,
    ...
    def _evaluate_structure_fitness(
        ...
        for test_input in test_inputs:
            if self.meta_prompt.mode == PromptMode.CODE:
                _, error_msg_delta = self.call_code_function(
                    test_input, code, file_path=None
                )
                if error_msg_delta == "":
                    passed_tests += 1
                else:
                ...
                _, error_msg_delta = self.call_prompt_function(
                    test_input, code, self.get_response
                )
                if error_msg_delta == "":
                    passed_tests += 1
                else:
    ...
    def call_prompt_function(
        self, test_input: Dict, code: Optional[str] = None, max_tries: int = 3
    ):  # TBD: Batch inference and pick the one which works
        if code is None:
            code = self.code

        error_msg = set()
        for i in range(max_tries):
            output_dict, error_msg_delta = call_func_prompt(
                test_input, code, self.get_response
            )
            if error_msg_delta == "":
                return output_dict, ""
    ...
    def call_prompt_functions(
        ...
        error_msg = set()
        output_dict, error_msg_delta = call_func_prompts(
            test_input, code, self.get_response, max_tries
        )
        if error_msg_delta == "":
            return output_dict, ""
        else:
    ...
    def call_code_function(
        ...
        output_value, error_msg = call_func_code(
            test_input, code, self.meta_prompt.func_name, file_path=file_path
        )
        output_name = self.meta_prompt.outputs[0]
        output_dict = {output_name: output_value}
        return output_dict, error_msg

    def call_prompt_function_parallel(
        self,
        test_inputs: List[Dict],
        ...
        if codes is None:
            codes = [self.code]
        return call_func_prompt_parallel(
            test_inputs, codes, max_tries, self.get_response
        )

    def call_code_function_parallel(
        self,
        test_inputs: List[Dict],
        codes: Optional[List[str]] = None,
        file_path: Optional[str] = None,
    ...
    def summarize_fitness(
        ...
        for code_index, code in enumerate(codes):
            for test_index, test_input in enumerate(test_inputs):
                if (
                    code_index in output_per_code_per_test
                    and test_index in output_per_code_per_test[code_index]
                ):
                    list_output = list(
                        output_per_code_per_test[code_index][test_index].values()
                    )
                    if self.meta_prompt.mode == PromptMode.CODE:
                ...
                structual_fitness_per_code_per_test[code_index][test_index] = (
                    structural_fitness
                )

        structual_fitness_per_code = defaultdict(float)
        for code_index in structual_fitness_per_code_per_test:
            structual_fitness_per_code[code_index] = sum(
                list(structual_fitness_per_code_per_test[code_index].values())
            ) / len(test_inputs)
        ...
        for code_index, code in enumerate(codes):
            for test_index, test_input in enumerate(test_inputs):
                if (
                    code_index in score_per_code_per_test
                    and test_index in score_per_code_per_test[code_index]
                ):
                    functional_fitness = score_per_code_per_test[code_index][test_index]
                else:
                    functional_fitness = 0
                functional_fitness_per_code_per_test[code_index][test_index] = (
                    functional_fitness
                )

        functional_fitness_per_code = defaultdict(float)
        for code_index in functional_fitness_per_code_per_test:
            functional_fitness_per_code[code_index] = sum(
                list(functional_fitness_per_code_per_test[code_index].values())
            ) / len(test_inputs)

        fitness_per_code = defaultdict(Fitness)
        for code_index in structual_fitness_per_code:
            fitness_per_code[code_index] = Fitness(
                structual_fitness_per_code[code_index],
                functional_fitness_per_code[code_index],
            )
    ...
    def _evaluate_fitness(
        ...
        if self.meta_prompt.mode == PromptMode.CODE:
            ...
            output_per_code_per_test, errors_per_code_per_test = (
                self.call_prompt_function_parallel(test_inputs, codes, max_tries)
            )
        else:
            raise ValueError(f"Unknown mode: {self.meta_prompt.mode}")
        ...
        for code_index in output_per_code_per_test:
            ...
            for test_index in output_per_code_per_test[code_index]:
                print(
                    f"Test {test_index}: {output_per_code_per_test[code_index][test_index]}"
                )
        ...
        score_per_code_per_test, evaluate_errors_per_code_per_test = (
            check_alignment_parallel(
                output_per_code_per_test,
                test_inputs,
                target_outputs,
                self.get_response,
        ...
        errors_per_code = combine_errors(
            evaluate_errors_per_code_per_test, errors_per_code_per_test
        )

        fitness_per_code = self.summarize_fitness(
            codes,
            score_per_code_per_test,
            output_per_code_per_test,
            test_inputs,
        ...
        # Get best fitness
        best_fitness = max(
            fitness_per_code.values(), key=lambda x: x(), default=Fitness(0.0, 0.0)
        )
        global_summary = (
    ...
    def _evaluate_fitness_sequential(
        ...
        for test_input, test_output in tqdm(test_cases, desc="Evaluating fitness"):
            if self.meta_prompt.mode == PromptMode.CODE:
                output_dict, error_msg_delta = self.call_code_function(
                    test_input, code, file_path=None
                )
                ...
                if error_msg_delta == "":
                    ...
                    is_aligned, error_msg_delta2 = check_alignment_sequential(
                        output_dict, test_output, self.get_response
                    )
                ...
                output_dict, error_msg_delta = self.call_prompt_function(
                    test_input, code, max_tries
                )
                if error_msg_delta == "":
                    compiled_tests += 1
                    is_aligned = check_alignment_sequential(
                        output_dict, test_output, self.get_response
                    )
                    if is_aligned:
                        passed_tests += 1
        ...
        global_summary = (
            f"--- Compiled {compiled_tests} out of {total_tests} test cases\n"
        )
    ...
    def batch_inference(
        ...
        if self.meta_prompt.mode == PromptMode.CODE:
            output_per_code_per_test, errors_per_code_per_test = (
                self.call_code_function_parallel(inputs, codes, timeout=timeout)
            )
        elif self.meta_prompt.mode == PromptMode.PROMPT:
            output_per_code_per_test, errors_per_code_per_test = (
                self.call_prompt_function_parallel(inputs, codes, max_tries)
            )
        else:
            raise ValueError(f"Unknown mode: {self.meta_prompt.mode}")
    ...
    def __call__(
        ...
        if self.meta_prompt.mode == PromptMode.CODE:
            output_value, err_msg = call_func_code(
                inputs,
                self.code,
                self.meta_prompt.func_name,
                file_path=None,
            ...
            }  # assuming single output for code-based node

        elif self.meta_prompt.mode == PromptMode.PROMPT:
            output_name = self.meta_prompt.outputs[0]

            if batch_inference:
                output_dict, err_msg = self.call_prompt_functions(
                    inputs, self.code, max_tries=max_attempts
                )
            else:
                output_dict, err_msg = self.call_prompt_function(
                    inputs, self.code, max_tries=max_attempts
                )
    ...
    def save(self, library_dir: str = "methods/nodes/", overwrite=False) -> None:
        node_data = {
            ...
            "test_cases": [
                {"input": test_case[0], "expected_output": test_case[1]}
    ...
    def load(
        cls,
        node_name: str,
        library_dir: str = "methods/nodes/",
        get_response: Optional[Callable] = get_openai_response,
    ) -> "EvolNode":
        node_path = os.path.join(library_dir, f"{node_name}_node.json")
        with open(node_path, "r") as f:
            node_data = json.load(f)
        meta_prompt = MetaPrompt.from_dict(
            node_data["meta_prompt"]
        )  # Assuming MetaPrompt has a from_dict method
        test_cases = [
            tuple([test_case["input"], test_case["expected_output"]])
            for test_case in node_data["test_cases"]
        ]
        node = cls(
    ...
    def query_nodes(
        self, top_k: int = 5, ignore_self: bool = False, self_func_name: str = None
    ) -> List["EvolNode"]:
        """
        Query nodes from library
        """
        query_engine = QueryEngine(
            ignore_self=ignore_self, self_func_name=self_func_name
        )
    ...
    def relevant_node_desc(self):
        ...
        return (
            ...
            + "If you intend to use this function, put the function calls into your generated function (assume the functions are already implemented and do not import them). Do not use it in a separate code block with your generated function. MAKE SURE THE FUNCTION IS USED AS A FUNCTION CALL SO IT CAN BE DETECTED BY AST\n"
        )

    @property
    def referrable_function_dict(self):
        referrable_function_dict = {
            node.meta_prompt.func_name: node.code for node in self.relevant_nodes
        }  # name to code of referrable functions
...
class PlanNode:
    ...
    def _evolve_plan_dict(
        ...
        if code == "":
            err_msg += (
                "No pseudo-code block found in the planning response: \n {response}\n"
            )
        ...
        plan_dict, err_msg_delta = check_n_rectify_plan_dict(
            plan_dict, self.meta_prompt
        )
        if err_msg_delta:
    ...
    def evolve_plan_dict_sequential(
        self, feedback: str = "", method: str = "i1"
    ):  # Deprecated
        err_msg = ""
        for i in range(self.max_attempts):
            ...
            if err_msg_delta == "":
                ...
                return plan_dict, ""
            err_msg += (
                f"Plan evolution {i} failed with error message: \n{err_msg_delta}\n"
            )
    ...
    def spawn_test_cases_majority(self, main_test_cases: list) -> tuple[bool, str]:
        def convert_unhash_to_hash(data):
            if isinstance(data, list):
                ...
                return frozenset(
                    (key, convert_unhash_to_hash(value)) for key, value in data.items()
                )
            else:
                return data
        ...
        def most_common(list_of_dicts):
            ...
            sorted_dicts_by_frequency = sorted(
                counter.items(), key=lambda x: x[1], reverse=True
            )
            sorted_unique_dicts = [
                convert_hash_to_unhash(d) for d, _ in sorted_dicts_by_frequency
            ]
            # pick the highest-count/earliest item
            return sorted_unique_dicts

        test_cases_dict, err_msg = spawn_test_cases(
            self.plan_dict, main_test_cases, get_multiple_response, unique=False
        )
        if test_cases_dict:
    ...
    def spawn_test_cases_sequential(
        self, main_test_cases: list
    ) -> tuple[bool, str]:  # Deprecated
        test_cases_dict, err_msg = spawn_test_cases(
            self.plan_dict, main_test_cases, self.get_response, SPAWN_TEST_MAX_TRIES
        )
        if test_cases_dict:
            self.test_cases_dict = test_cases_dict
            print(f"Spawned {len(test_cases_dict)} test cases for all sub-nodes")
    ...
    def spawn_test_cases(
        self, main_test_cases: list, batch_size: int = 1
    ) -> tuple[bool, str]:
        test_cases_dict, err_msg = spawn_test_cases(
            self.plan_dict, main_test_cases, self.get_response, SPAWN_TEST_MAX_TRIES
        )
    ...
    def evolve_sub_nodes(
        self, method: str = "i1", batch_size: int = NODE_EVOLVE_BATCH_SIZE
    ):
        """
        ...
        for i, node_dict in enumerate(self.plan_dict["nodes"]):
            ...
            if "fitness" in node_dict and "code" in node_dict:
                ...
                print(
                    f"🎲 :: Evolving {node.meta_prompt.func_name} ... ({i+1}/{len(self.plan_dict['nodes'])})"
                )
                node.evolve(
                    method, replace=True, max_tries=2, num_runs=2, batch_size=batch_size
                )  # It's funny how 30+ sec could elapse before llm inference ... (collecting prompts ?? wtf is taking so long ??)
    ...
    def _spawn_nodes(
        self, plan_dict: Dict
    ):  # bad idea since test cases are not grounded ...
        """
    ...
    def load(
        ...
        meta_plan = MetaPlan.from_dict(
            plan_data["meta_prompt"]
        )  # Assuming MetaPlan has a from_dict method
    ...
    def relevant_node_desc(self):
        if len(self.relevant_nodes) == 0:
            return ""
        return (
            "You could call these available functions without defining them:\n"
