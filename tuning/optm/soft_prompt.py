...
def load_tf_data(data_path: str = BASE_PATH, split_ratio: float = 0.8):
    ...
    train_indices, test_indices = train_test_split(
        range(len(data["prompt"])), test_size=0.2, random_state=42
    )
...
def tf_label_metric(
    target_labels: Union[str, list], pred_labels: Union[str, list], beta: float = 5
):
    """
    ...
    true_pos = sum(
        1 for t, p in zip(target_labels, pred_labels) if t == "no" and p == "no"
    )
    ...
    weighted_score = (
        ((1 / (1 + beta)) * no_precision + (beta / (1 + beta)) * no_recall)
...
def format_prompt_instruction_tuned(
    prompt: str, comment: str, label: str, tokenizer, previous_messages: list = []
):
    """
    ...
    fillin_messages = previous_messages + [
...
class ClsCommentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        query_prompt, response_prompt = format_prompt_instruction_tuned(
            prompt, comment, label, self.tokenizer, previous_messages=[]
        )
...
class SoftPromptLLM(nn.Module):
    def __init__(
        self, model, tokenizer, n_learnable_tokens, initialize_from_vocab=False
    ):
        super().__init__()
        self.model = model
        self.tokenizer = tokenizer
        ...
        if initialize_from_vocab:
            ...
            self.soft_prompts = nn.Parameter(
                model.get_input_embeddings()(init_prompt_ids), requires_grad=True
            )
        else:
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            prefix_mask = torch.ones(
                batch_size, self.n_learnable_tokens, device=attention_mask.device
            )
        ...
        if labels is not None:
            prefix_labels = torch.full(
                (batch_size, self.n_learnable_tokens),
                -100,
                dtype=labels.dtype,
    ...
    def generate(self, prompts: Union[str, List[str]], max_new_tokens: int = 100):
        ...
        prefix_mask = torch.ones(
            batch_size, self.n_learnable_tokens, device=self.device
        )
        attention_mask = torch.cat([prefix_mask, attention_mask], dim=1)
        ...
        for output in outputs:
            ...
            text = self.tokenizer.decode(
                output[self.n_learnable_tokens :], skip_special_tokens=True
            )
...
def check_and_handle_nans(loss, model, pred_logits, batch_idx, optimizer):
    ...
    if torch.isnan(loss) or torch.isinf(loss):
        ...
        print(
            f"Soft prompts stats: min={model.soft_prompts.min():.4f}, max={model.soft_prompts.max():.4f}"
        )
...
def _label_metric(target_label: str, generated_response: str) -> float:
    ...
    if "decision" not in parsed_response:
        ...
        if generated_label.lower() != target_label.lower():
            return (
                0.0,
                f"Target label is '{target_label}', but generated label is '{generated_label}'",
            )
        else:
            return 1.0, "Perfect!"
...
def test_model(model_with_soft_prompt, tokenizer, test_data, batch_size=24):
    ...
    for i in tqdm(range(0, data_len, batch_size), desc="Testing model"):
        ...
        for prompt, comment, label in zip(prompts, comments, labels):
            query_prompt, response_prompt = format_prompt_instruction_tuned(
                prompt, comment, label, tokenizer, previous_messages=[]
            )
            query_prompts.append(query_prompt)

        generated_responses = model_with_soft_prompt.generate(
            query_prompts, max_new_tokens=600
        )

        for response, label in zip(generated_responses, labels):
            score, err_msg = label_metric(
                response, label
            )  # Now properly unpacking both return values
            scores.append(score)
            err_msgs.append(err_msg)
...
def train_soft_prompt(
    ...
    for epoch in range(num_epochs):
        ...
        progress_bar = tqdm(
            train_dataloader, desc=f"Epoch {epoch+1}/{num_epochs} - Loss: N/A"
        )
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            # NaN checking
            should_continue = check_and_handle_nans(
                loss, model, pred_logits, batch_idx, optimizer
            )
            if not should_continue:
                raise ValueError(
                    "NaN/Inf loss detected in first batch - training stopped"
                )
            ...
            progress_bar.set_description(
                f"Epoch {epoch+1}/{num_epochs} - Loss: {running_avg:.4f}"
            )
...
def train_epoch(
    ...
    def handle_gradients(batch_idx: int, loss: torch.Tensor) -> bool:
        ...
        try:
            ...
            if torch.isnan(grad_norm) or grad_norm > max_grad_norm * 10:
                print(
                    f"Gradient norm too large: {grad_norm}, skipping batch {batch_idx}"
                )
                optimizer.zero_grad()
                return False
    ...
    for batch_idx, batch in enumerate(progress_bar):
        try:
            ...
            if (batch_idx + 1) % accumulation_steps == 0:
                if handle_gradients(
                    batch_idx, loss * accumulation_steps
                ):  # Pass tensor loss
...
def run_prompt_tuning_pipeline(
    ...
    train_dataloader = DataLoader(
        tf_dataset, batch_size=config_dict["batch_size"], shuffle=True
    )
    test_dataloader = DataLoader(
        testset, batch_size=config_dict["batch_size"], shuffle=True
    )
    ...
    for epoch in range(num_epochs):
        train_loss, eval_loss = train_epoch(
            ...
            tokenizer,
            device,
            accumulation_steps=config_dict["accumulation_steps"],
        )
...
def evaluate_model_outputs(
    trained_model, tokenizer, cap_num: int = 30, config_dict: dict = None
) -> list:
    generated_responses = []
    ...
    with torch.no_grad():
            ...
            if isinstance(trained_model, SoftPromptLLM):
                ...
                generated_responses.extend(
                    trained_model.generate(
                        batch_prompts,
                        max_new_tokens=config_dict.get("max_new_tokens", 2024),
                    )
                )
            else:
        ...
        fitness, err_msgs = label_metric(
            test_data["label"][:cap_num], generated_responses
        )
...
def run_token_tuning_pipeline(
    ...
    model, tokenizer = load_hf_model_precise(
        config_dict.get("model_name", "Qwen/Qwen2.5-0.5B-Instruct")
    )
    ...
    orig_fitness, orig_responses, orig_err_msgs = evaluate_model_outputs(
        model, tokenizer, cap_num, config_dict={}
    )
    ...
    tf_dataset = ClsCommentDataset(load_tf_data, tokenizer, train=True, max_length=8062)
    testset = ClsCommentDataset(load_tf_data, tokenizer, train=False, max_length=8062)
    ...
    train_dataloader = DataLoader(
        tf_dataset, batch_size=config_dict.get("batch_size", 24), shuffle=True
    )
    ...
    init_fitness, init_responses, init_err_msgs = evaluate_model_outputs(
        model_with_soft_prompt, tokenizer, cap_num, config_dict
    )
    ...
    token_fitness, token_responses, token_err_msgs = evaluate_model_outputs(
        trained_model, tokenizer, cap_num, config_dict
    )
